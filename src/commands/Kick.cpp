...
int	cmd::kick(const vector<string> &input, Person & from)
{
	...
	else if (!searchCh(input[1], from))
	{
		Response::withCode(ERR_NOSUCHNICK).to(from).addContent(input[1] + NO_SUCH).send();
		return (-1);
	}
	else if (!Compare(start.getChannel(input[1])[0]->getNickName(), from.getNickName(), 1))
	{
	...
	{	
		...
		if (channel[i]->getNickName() == to->getNickName())
		{
			...
			if (channel.size() == 0)
				...
				modeHandler(channel, input[1], channel[i]->getNickName());
			i--;
		}
	}
