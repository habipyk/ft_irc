...
void	Server::setHostname()
{
	...
	handle_socket(return_number, "gethostname");
	this->hostname = hostname_c;
}

void	Server::makeUserVanish(int fd)
{
...
void	Server::deleteUser(const string &channel, Person &user)
{
	int fd = user.getFd();
	...
	for (vector<Person *>::size_type i = 0; i < userList.size(); i++)
	{
		...
		if (target && target->getFd() == fd)
		{
			...
			if (userList.size() == 0)
				...
				modeHandler(userList, channel, userList[i]->getNickName());
...
void	Server::userHandler(const string &group, Person &user) 
{
	if (!searchCh(channels[group], user.getNickName()))
	...
	Response::createReply(RPL_NAMEREPLY).to(user).addContent("= " + group + Visualizer(channels[group])).send();
