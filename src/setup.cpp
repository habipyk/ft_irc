...
fp_command	selCommand(vector<string> &input, const Person &user)
{
	...
	for (i = -1; i < 10; ++i)
		if (Compare(str[i], input[0], str[i].size() >= 1) || 
				Compare(str[i], input[1], str[i].size() >= 2))
			break;
...
void	Server::checkInput(int fd, const string &input)
{
	fp_command 		func;
	string			str;
	vector<string>	commands;
	Person			*person = makeUser(fd);
	commands = split_input(trimString(input));
	if ((func = selCommand(commands, *person)) != NULL)
	{
		printerCl(input, *person);
				...
				else
				{
					...
					if (readed > 0){
						...
						if (endsWith(lines[pollfds[i].fd],"\n")){
							checkInput(pollfds[i].fd, lines[pollfds[i].fd]);
							lines[pollfds[i].fd] = "";
						}
					}
